...
const CONFETTI_COLORS = ['#f72585', '#7209b7', '#3a0ca3', '#4361ee', '#4cc9f0', '#f77f00', '#fcbf49'];
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-foreground/90">
        ...
        <div className="flex flex-col items-center gap-6 z-10">
          <h1 className="pixel-text text-6xl md:text-8xl font-bold text-destructive" style={{ animation: 'shake 0.5s ease-in-out infinite' }}>
            YOU'RE FIRED!
          </h1>
          <PartyPopper size={64} className="text-warning" />
          <p className="text-xl text-primary-foreground font-bold text-center max-w-md">
            🎉 Congratulations! You successfully avoided all productivity and earned your freedom! 🎉
          </p>
          <button className="xp-button-primary text-lg px-8 py-2" onClick={onRestart}>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-card">
      <div className="flex flex-col items-center gap-6">
        ...
        <p className="text-lg text-muted-foreground text-center max-w-md leading-relaxed">
          You were <strong>too good</strong> at your job. You've been promoted to<br />
          <span className="text-destructive font-bold text-xl">Senior Vice President of Meetings</span>
        </p>
        <p className="text-sm text-muted-foreground italic">You are trapped here forever.</p>
        <button className="xp-button-primary text-lg px-8 py-2" onClick={onRestart}>
          Try Again (Escape)
        </button>
      </div>

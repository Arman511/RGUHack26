...
const FALLBACK_VALID_WORDS = [
  "SYNCS","AGILE","PIVOT","SCRUM","EPICS","FLEET","RAPID","SLACK","STAND","FOCUS",
  "ALIGN","SCOPE","PATCH","BRICK","CRANE","GLOBE","HOUSE","LIGHT","MONEY","POWER",
  "QUEST","RAISE","SMART","THINK","ULTRA","VALVE","WATER","YIELD","BLAST","CHARM",
  "DRAFT","EIGHT","FLAME","GRAPE","HASTE","INPUT","JUDGE","KNEEL","LEAPS","MANGO",
  "NOBLE","OCEAN","PLUMB","QUITE","ROUND","SHARP","TOWER","UNITE","WATCH","WORLD",
  "BRAIN","CLOUD","DAILY","EXTRA","FRESH","GRAIN","TREND","TRACK","TRADE","TRAIN",
  "TRAIL","TRIAL","TWEAK","SWARM","SPARK","SPACE","SOLVE","SOLID","SHIFT","SHARE",
  "SHAPE","SERVE"
];

const TARGET_WORDS = [
  "SYNCS","AGILE","PIVOT","SCRUM","EPICS","SLACK","STAND","ALIGN","SCOPE","TREND"
];
...
export const WordleGame: React.FC<WordleGameProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const loadWords = async () => {
      try {
        ...
        const words = text.split(/\r?\n/).map(w=>w.trim().toUpperCase()).filter(w=>/^[A-Z]{5}$/.test(w));
  ...
  },[current,guesses,done,target,validWords,updateLetterStates,onComplete]);
  ...
  const getColor = (letter:string,index:number,row:string)=>{
    ...
    if(result[index]==="correct") return "bg-success text-success-foreground";
    if(result[index]==="present") return "bg-warning text-warning-foreground";
        ...
        {rows.map((row,ri)=>(
          <div key={ri} className="flex gap-1">
            {[0,1,2,3,4].map(ci=>{
              ...
              const colorClass = isCurrentRow ? "bg-card border border-border" : getColor(letter,ci,row);
              return (
                <div key={ci} className={`w-9 h-9 flex items-center justify-center text-sm font-bold border ${colorClass}`}>
                  {letter}
                </div>
              );
            })}
